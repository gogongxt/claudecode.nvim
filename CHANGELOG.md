...
- The session tab bar no longer hides sessions when the terminal window is narrow. With several Claude sessions open, the winbar's overflow is right-aligned, which truncated from the left and dropped the earliest sessions entirely. The tab bar now renders a contiguous _sliding window_ of sessions (like a bufferline/tabline plugin): whole tabs are dropped rather than name-truncated, `‹`/`›` markers indicate folded sides and are clickable to scroll the window one tab at a time, and the active session is always kept inside the window (it scrolls in only when it leaves the visible range; switching among already-visible sessions causes no jump). An explicit `‹`/`›` scroll lets you browse past the active session until the next session switch. Applies to both the split (winbar) and float render paths.
- Session names are now shown in full in the tab bar — the previous 12-character cap (with `…`) is removed. Long names render whole when their tab is visible; the sliding window drops whole tabs to fit rather than truncating names.
- `:ClaudeCodeRenameSession` (with no argument) now opens its prompt pre-filled with the active session's current name, so you can edit the name in place rather than retyping it. The prompt is also forced into Insert mode under `vim.ui.input` overrides that open a buffer-backed float in Normal mode (e.g. noice.nvim), where typed keys were previously interpreted as motions; the native command-line prompt was already typeable and is unaffected.
